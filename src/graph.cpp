...
//     double trash;
    ...
    for (int i = 0; i < E; i++)
    {
        ...
        double flow;
        #ifdef RADIUS /*EU*/
        in >> e[0] >> e[1] >> radius >> flow;
        #else
        in >> e[0] >> e[1];
        #endif
        //if (e[0] == 217 && e[1] == 4456)
        //    break;
        insertEdgeGraph(e[0],e[1],0,radius,flow);
        insertEdgeGraph(e[1],e[0],1,radius,flow);
    }
    in.close();

    // Calcular o numero de terminais da arvore
    calc_terminals();
...
Node::Node (int id, int type, double x, double y, double z, double v)
{
    this->type = type;
    this->id = id;
    this->x = x;
...
Edge::Edge (int id, double w, double r, double f, Node *dest)
{
	this->id = id;
	this->w = w;
    this->radius = r;
...
void Graph::insertEdgeGraph (int id_1, int id_2, int entrada,  /*EU*/ double radius, double flow)
{
	Node *ptr1, *ptr2;
    ...
    Edge *edge = new Edge(id_2,norm,radius,flow,ptr2);
    edge->entrada = entrada;
    // First edge
    if (ptr1->edges == NULL)
    {
        ptr1->edges = edge;
        ptr1->edges->radius = radius;
        ptr1->edges->flow = flow;// EU
    }
...
//             insertNodeGraph(1,p);
//             insertEdgeGraph(ptr->id,total_nodes-1,1);
//             insertEdgeGraph(total_nodes-1,ptr->id,0);
//         }
//         ptr = ptr->next;
//     }
// }
...
// 		#endif
		...
		while (ptrl != NULL)
		{
             ...
             printf(" --> || %d (%d) Flow = %lf ||",ptrl->id,ptrl->entrada,ptrl->flow);
...
void Graph::calc_terminals () {
    Node *ptr = this->listNodes;
    this->nterm = 0;
    while (ptr != NULL) {
        if (ptr->num_edges == 1) {
            this->nterm++;        
